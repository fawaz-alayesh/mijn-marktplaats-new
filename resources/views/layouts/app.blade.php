...
<body class="antialiased bg-amber-50">
  <!--  navmenu -->
  <nav class="sm:bg-transparent bg-transparent border border px-2 md:px-4 py-1 dark:bg-gray-900">
      ...
      <div class="flex items-center pl-2 pr-2">
        ...
        <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded shadow dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
          ...
          <ul class="py-1" aria-labelledby="user-menu-button">
            ...
            <li>
              <a href="{{ route('updateprofile') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Profile bewerken</a>
            </li>
            <li>
              <a class="block px-4 py-2 text-sm text-red-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white" href="{{ route('logout') }}" onclick="event.preventDefault();document.getElementById('logout-form').submit();">
                {{ __('Logout') }}
              </a>
            </li>
          </ul>
    ...
    <nav class="border border shadow py-2">
      <div class="max-w-screen-xl px-4 py-1 mx-auto md:px-6">
        ...
        <div class="md:flex items-center justify-between pb-3">
          <!-- if user logged in (Tel) responsive -->
          @if(Auth::check())
          ...
          <ul class="flex flex-row items-center mt-2 w-full space-x-3 md:space-x-5 text-sm font-medium">
            ...
            <li class="{{ Request::is('advertentie') ? 'active_nav' : '' }}">
              <a href="{{ route('advertentie.index') }}" class="text-gray-900 dark:text-white hover:text-purple-600">Advertentie's</a>
            </li>
            <li class="{{ Route::currentRouteName() == 'advertentie.category' ? 'active_nav ' : '' }}">
              ...
              <div id="mega-menu-icons-dropdown" class="absolute z-50 grid hidden p-2 w-auto grid-cols-1 text-sm bg-white border border-gray-100 rounded-lg shadow-md dark:border-gray-700 dark:bg-gray-700">
                <div class="p-4 pb-0 text-gray-900 md:pb-4 dark:text-white">
                  <ul class="grid grid-rows-3 grid-flow-col gap-6" aria-labelledby="mega-menu-icons-dropdown-button">
                    @foreach($categories as $category)
                    <li class="hover:bg-purple-100 px-2 py-1 px-2 py-1 {{ url()->current() == route('advertentie.category', ['category' => $category->id]) ? 'bg-purple-200' : '' }}">
                      <a href="{{ route('advertentie.category', ['category' => $category->id]) }}" class="text-gray-500 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-500 group">
                        {{$category->name}} ({{$category->advertenties()->count()}})
                      </a>
                    </li>
                    @endforeach
                  </ul>
            ...
            <li class="{{ Request::is('about') ? 'active_nav' : '' }}">
              <a href="{{ route('about') }}" class="text-gray-900 dark:text-white hover:text-purple-600">Over ons</a>
            </li>
            <li class="{{ Request::is('contact') ? 'active_nav' : '' }}">
              <a href="{{ route('contact') }}" class="text-gray-900 dark:text-white hover:text-purple-600">Contact</a>
            </li>
          </ul>
          <!-- if user logged in -->
          @if(Auth::check())
  ...
  <!-- Main content -->
  <main class="relative flex justify-center min-h-screen w-full my-4 sm:pt-0">
    @yield('content')
  </main>
  <!-- End Main content -->
  </div>
  <!-- End Body Color Effect -->
